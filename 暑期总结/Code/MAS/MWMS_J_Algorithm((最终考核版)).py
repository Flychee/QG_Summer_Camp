...
class MWMS_J:
    def __init__(self, DP_flag, rc=0.5, alpha=0.5):
        ...
        self.location_address = 'MWMS_J_10_200_location'  # the name is MWMS_J_x_xxx_location
        self.adjacency_address = 'MWMS_J_10_200_adjacency'  # the name is MWMS_J_x_xxx_adjacency
        self.npy_path = r'C:\Users\86177\Desktop\QG_Summer_Camp\Last Assessment\Algorithm\数据集\Agents-200\010log_neat_radiation_200\010log_neat_radiation_200.npy'  # the address of the .npy used to create the topology matrix
        self.file = ""
    ...
    def W_J(self, A, M):  # MWMS-J加权混合矩阵
        return (1 - self.alpha) * M + self.alpha * A
    ...
    def MWMS_J(self, W_r):
        self.G[1] = np.dot(np.dot(np.linalg.pinv(np.eye(np.shape(W_r)[0]) + MWMS_J.D(self, W_r)),
                             np.eye(np.shape(W_r)[0]) + W_r), self.G[1])
    ...
    def main(self):
        ...
        adjacency_list = []
        time = 0
        MWMS_J.G_create(self)
        while 1:
            MWMS_J.G_merge(self, MWMS_J.A(self, self.G_original, 0.05 * self.rc))  # 收敛判定精度
            test1 = self.G
            MWMS_J.G_merge(self, MWMS_J.A(self, self.G_original, self.rc))  # 通信距离
            test2 = self.G
            if len(test1[0]) == len(test2[0]):
                MWMS_J.lct_list(self, location_list, time)
                A_rc = MWMS_J.A(self, self.G_original, self.rc)
                MWMS_J.ajc_list(self, A_rc, adjacency_list, time)
                print(self.location_address[: -9] + "聚类完毕")
                break
            A_cnt = MWMS_J.A(self, self.G_original, 0.000001 * self.rc)  # 去重精度
            A_rc = MWMS_J.A(self, self.G_original, self.rc)
            MWMS_J.G_merge(self, A_cnt)
            A_trad = MWMS_J.A(self, self.G[1], self.rc)
            MWMS_J.TPfig(self, A_trad, start=-6, end=6, time=time)
            MWMS_J.lct_list(self, location_list, time)
            MWMS_J.ajc_list(self, A_rc, adjacency_list, time)

            M = MWMS_J.motif(self, A_trad)  # MWMS-J
            W_temp = MWMS_J.W_J(self, A_trad, M)
            W_r_temp = MWMS_J.W_r(self, W_temp)
            MWMS_J.MWMS_J(self, W_r_temp)
            ...
            print(time)
            time += 1
        # MWMS_J.file_write(self, location_list, adjacency_list)
        return "SUCCESS"
